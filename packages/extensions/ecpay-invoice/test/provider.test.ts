...
function provider() {
  return createEcpayInvoiceProvider(createTestExtensionContext({ extensionId: 'ecpay-invoice', config: ecpayInvoiceConfig.parse({ environment: 'stage' }), secrets, now: () => new Date('2026-08-25T10:00:00.000Z') }));
}
...
describe('ECPay B2C invoice provider', () => {
  it('encrypts the documented tax-inclusive issue payload and normalizes an issued result', async () => {
    ...
    await expect(provider().issue(issueInput)).resolves.toEqual({ status: 'issued', providerRef: 'AB12345678', invoiceNumber: 'AB12345678', invoiceDate: '2026-08-25 18:00:00' });
  ...
  it('checks a donation love code before issuance and maps carrier choices exactly', async () => {
    ...
    vi.stubGlobal('fetch', request);
    await expect(provider().validateLoveCode('168001')).resolves.toBe(true);
  ...
  it('uses the B2C Invalid endpoint with an invoice date only for a void', async () => {
    ...
    vi.stubGlobal('fetch', request);
    await expect(provider().void({ invoiceId: issueInput.invoiceId, reference: 'void:invoice', providerRef: 'AB12345678', invoiceNumber: 'AB12345678', invoiceDate: '2026-08-25 18:00:00', reason: 'full refund' })).resolves.toEqual({ status: 'voided', providerRef: 'AB12345678' });
    const [url, init] = request.mock.calls[0] as unknown as [string, RequestInit];
