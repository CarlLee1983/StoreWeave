...
export function toArticleDto(row:ContentArticleRow):ArticleDto{return{id:row.id,kind:row.kind as ArticleKind,slug:row.slug,title:row.title,summary:row.summary,section:row.section,body:row.body??[],imageKey:row.imageKey,position:row.position,status:row.status as ArticleDto['status'],publishedAt:row.publishedAt,createdAt:row.createdAt,updatedAt:row.updatedAt};}
...
export class ContentRepository {
 ...
 async listPublished(db:DrizzleDb,kind:string,limit:number){const rows=await db.select().from(contentArticles).where(and(eq(contentArticles.kind,kind),eq(contentArticles.status,'published'))).orderBy(...listingOrder).limit(limit);return rows.map(toArticleDto);}
