...
export const createPromotionHandler = async (
  ...
  const row = await repository.insert(ctx.tx, {
    ...
    autoIssueValidDays: input.autoIssueValidDays ?? null,
...
export const updatePromotionHandler = async (
  ...
  const row = await repository.update(ctx.tx, id, {
    ...
    ...(patch.requiresCoupon === undefined ? {} : { requiresCoupon: patch.requiresCoupon }),
    ...(patch.autoIssue === undefined ? {} : { autoIssue: patch.autoIssue }),
    ...(patch.tierNames === undefined ? {} : { tierNames: patch.tierNames }),
    ...(patch.autoIssueValidDays === undefined ? {} : { autoIssueValidDays: patch.autoIssueValidDays }),
