...
/** Select one already-ready B10 asset, or clear the reference without touching legacy image_key. */
export const setArticleMediaInput=z.object({id:z.string().uuid(),mediaAssetId:z.string().uuid().nullable()}).strict();
export const publishedMediaInput=z.object({mediaAssetId:z.string().uuid()}).strict();
export const publishedMediaOutput=z.object({mediaAssetId:z.string().uuid()});
...
export const listPublishedArticlesInput=z.object({kind:articleKind,limit:z.coerce.number().int().min(1).max(100).default(12),offset:z.coerce.number().int().min(0).default(0)}).strict();
export const listPublishedArticlesOutput=z.object({items:z.array(articleDto),total:z.number().int().nonnegative()});
/** Discovery feeds never use the staff list: this is published-only without a kind filter. */
export const listPublishedSiteContentInput=z.object({limit:z.coerce.number().int().min(1).max(100).default(100),offset:z.coerce.number().int().min(0).default(0)}).strict();
export const listPublishedSiteContentOutput=z.object({items:z.array(articleDto),total:z.number().int().nonnegative()});
