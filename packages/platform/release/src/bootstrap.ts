...
import { assertReleaseComposition, type RuntimeReleaseDefinition } from './runtime';
...
export interface ReleaseBootstrapResult<C extends BaseConfig> {
  release: RuntimeReleaseDefinition<C>;
  runtime: Runtime<C>;
...
export async function bootstrapRelease<C extends BaseConfig>(
  release: RuntimeReleaseDefinition<C>, options: BootstrapOptions,
  ...
  const logger = createLogger({
    ...
    destination: config.logging.destination === 'stdout' ? options.logDestination ?? 'stdout' : config.logging.destination,
    file: config.logging.file,
    name: options.loggerName,
  });
  try {
    const theme = Object.hasOwn(release.availableThemes, config.theme.id) ? release.availableThemes[config.theme.id] : undefined;
    ...
    runtime.close = () => closing ??= withCleanupDeadline(config.shutdown.timeoutMs, () => closeInReverse([() => logger.close(), closeRuntime]));
