...
import { collectPages, composeRuntimeModules, parseScheduleSpec, scheduleOccurrencePayload } from '@storeweave/kernel';
import type { RuntimeReleaseDefinition } from './runtime';
...
/** Release-scoped, declaration-only module contract checks. */
export function runModuleContractChecks<C extends BaseConfig>(
  release: RuntimeReleaseDefinition<C>, moduleName: string,
): ModuleContractCheck[] {
  ...
  push('jobs declare a payload contract', (mod.jobs ?? []).filter(job => !job.jobContractV1).map(job => job.type));
  push('scheduled jobs accept the payload their schedule produces', (mod.jobs ?? []).filter(job => {
    ...
    return !job.jobContractV1.versions[job.jobContractV1.currentVersion]?.safeParse(payload).success;
  }).map(job => job.type));
...
export function assertModuleContract(checks: readonly ModuleContractCheck[]): void {
  ...
  if (failed.length > 0) throw new Error(`Module contract violations:\n${failed.map(check => ` - ${check.name}${check.message ? `: ${check.message}` : ''}`).join('\n')}`);
}
