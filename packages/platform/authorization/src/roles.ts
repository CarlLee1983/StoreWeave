...
export const BUILT_IN_ROLES: Record<string, readonly string[]> = {
  ...
  customer: ['catalog:read', 'inventory:read', 'order:read', 'order:write', 'refund:read', 'rma:read', 'rma:create', 'promotion:quote', 'shipping:read', 'customer:read', 'customer:write', 'cart:read', 'cart:write', 'content:public-read', 'contact:submit'],
};
...
export const BASE_ROLES: ReleaseRoleCatalog = {
  ...
  staff: { permissions: ['users:read', 'jobs:read', 'jobs:write', 'storage:read', 'storage:write', 'storage:delete', 'storage:share'], tokenAllowed: true, account: operatorAccount },
  readonly: { permissions: ['users:read', 'jobs:read', 'storage:read'], tokenAllowed: true, account: operatorAccount },
