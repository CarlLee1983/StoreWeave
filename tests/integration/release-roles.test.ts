...
describe('release-owned roles', () => {
  ...
  it('Base uses only its own account and actor permissions', async () => {
    ...
    expect(session?.actor).toMatchObject({ type: 'user', permissions: ['users:read', 'jobs:read', 'storage:read', 'media:read', 'notifications:read', 'notifications:inbox', 'content:read', 'contact:read', 'site:public-read'] });
