...
const baseSets = [platformMigrations, cacheMigrations, identityMigrations, storageMigrations];
...
describe('release transitions', () => {
  ...
  it('adopts the full pinned Commerce legacy state before forward Base migrations', async () => {
    ...
    expect(prepared.manifest.owners.filter(entry => entry.state === 'active')).toHaveLength(5);
