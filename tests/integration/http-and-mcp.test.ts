...
describe('REST 介面', () => {
  it('retains exact selected Commerce route identities with MCP on and off', async () => {
    ...
    expect(catalog.storeweaveHttpCatalog?.filter(route => !route.kind.startsWith('static-'))).toHaveLength(198);
    ...
    try {
      ...
      expect(withoutMcpCatalog.storeweaveHttpCatalog?.filter(route => !route.kind.startsWith('static-'))).toHaveLength(196);
