...
async function copyAndVerify(source: NodeJS.ReadableStream, target: string, expected: { byteSize: number; sha256: string }): Promise<void> {
  ...
  try {
    ...
    if (byteSize !== expected.byteSize || digest.digest('hex') !== expected.sha256) throw new Error('Storage object bytes do not match its persisted digest');
    sync(temporary);
 ...
 */
export async function restoreStorageBackup(runtime: Runtime, directory: string, backup: StorageBackup): Promise<void> {
  const root = resolve(directory);
  for (const entry of backup.objects) {
    const file = join(root, entry.file);
    await runtime.storage.restoreExact({
      namespace: entry.namespace, id: entry.id, storageKey: entry.storageKey,
      byteSize: entry.byteSize, sha256: entry.sha256, open: () => createReadStream(file),
    });
  }
}
