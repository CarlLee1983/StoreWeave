...
| `AC-003` | concurrency integration test | `booking-e2e.test.ts` late/excess callbacks; `booking-reservation.test.ts` lifecycle races | competing payment lifecycle events | winner/refund/no revival pass; distinct Late Payment operator notification missing |
...
* `AC-003` remains open: the Late Payment test observes a notification link after cancellation, but that link can belong to the earlier cancellation. Booking currently has no Late Payment notification event, template, or operator recipient contract. Refund and Admin evidence pass; a Booking Reservation Story must define the recipient and add distinct, durable operator notification evidence before this criterion is checked.
* `make verify` passed on the test-source revision `52bf57d` (115 integration files, 1012 integration tests; unit and Admin gates also passed). This later change only corrects the acceptance record and its readiness digest.
