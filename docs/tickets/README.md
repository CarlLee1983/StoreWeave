...
由 `docs/specs/` 的五份規格拆出，共 49 張，之後補了第 50、51、52 張。依相依順序編號。詞彙見根目錄 `CONTEXT.md`，
決策見 `docs/adr/`（本批為 0013、0014、0015，並補充了 0009）。

**已完成**：01–23、25–52。定價引擎、顧客身分、購物車、優惠券與行銷碼、
...
等一個 minor 週期」：repo 內已無訂閱者（2026-08-23 完整盤點過，記在工單 24），
但外部部署未知，而那不是查得到的事——投遞是行程內的，沒有 webhook，
直接讀 outbox 的下游不會留下痕跡。代價是過渡期每張訂單多發 3 筆 outbox。要下線前再問一次。
