...
- [x] 商品列可展開或開啟編輯表單，帶入現值；未修改的欄位不送出
- [x] 售價維持 cents 整數驗證，與建立表單同一套規則，不另寫一份
- [x] 上下架是明確的狀態轉換，不是自由下拉：draft→active→archived 各自有按鈕；active 另給「下架回草稿」，
      因為收回去改與退役是兩件事。轉換表只在 UI，command 不強制——理由記在 ADR 0032
- [x] 已下架商品不影響既有訂單的價格快照——UI 要講清楚這件事，避免誤以為改價會回溯
- [x] 以 jsdom 測試覆蓋帶入現值、部分欄位更新與狀態轉換
