...
**Status:** done（2026-09-07；本機完整驗收，Sol/high Standards／Spec PASS）。2026-09-15 回填：改動已在 `main`，本批後台工作的落地 commit 為 `5bac79d feat(admin): add operations workbench and query foundation`（2026-09-08），B13 的 `d830c30` 另補上依權限組裝的導覽與自助頁；兩者皆經 `git merge-base --is-ancestor` 確認為 HEAD 的祖先。原本的「未 commit／push」註記寫於提交之前，已過時。
