...
**Status:** implementation complete — live carrier callback verification remains UAT-gated by 58.

- [x] callback 驗簽、冪等與狀態單向推進；原始 payload／狀態碼僅保留在營運記錄
- [x] 無 callback 的 carrier 有可排程、可觀測、可重試的查件工作
- [x] 訂單成立、付款完成、出貨、到貨各在正確領域事件後通知一次；發送失敗可重試但不回滾交易
