...
- [x] 六處以 `msg` 精確比對的過濾不再假設快路徑（工單 53 的 level 表照舊）
- [x] 「成功那一行不該出現」那條改成連 `slow ` 那個變體也擋得住——它現在會假綠
- [x] 拍板 level 斷言留在哪一層（下方「拍板」）
- [x] 在「每次呼叫都算慢」的條件下驗過（下方「怎麼證明它有效」）
...
**level 斷言留在整合測試，但改成綁在訊息上，而不是綁在「這次跑得夠快」上。**

上面那段傾向的做法是整合測試乾脆不驗 level。實作時發現有更好的：訊息的 `slow ` 前綴
與 level 都由 `logBusCall` 裡的**同一個 `slow` 變數**決定，所以

> 訊息說慢 → level 必須是 `warn`；訊息沒說慢 → level 必須是那一格的快路徑值

是一個恆成立的關係。耗時控制不了，這個關係控制得了。斷言換成這個之後，
flake 消失而 level 那一格**仍然驗得到**——不必為了不 flake 就把覆蓋率讓掉。

`expectLevelMatchesSpeed(line, fastLevel)` 就是這件事，六處共用。過濾則走
`isBusMessage(msg, base)`：快慢兩個變體都算同一件事。

## 怎麼證明它有效

不能等機器剛好忙。做法是把 `SLOW_CALL_MS` 暫時改成 0，讓**每一次**呼叫都走 slow 那條路，
兩個版本各跑一次：

| | `SLOW_CALL_MS = 0` |
| --- | --- |
| 改之前 | 6 條全紅（正是線上看到的 `expected [] to have a length of 1 but got +0`） |
| 改之後 | 8 條全綠 |

閾值改回 500 之後兩者都綠。這個手法沒有留在 repo 裡——它是一次性的驗證，
留下來就等於把常數開成設定，而那正是上面拒絕的事。

`isBusMessage` 自己有一組純函式測試（快慢都算數、不同的事就是不同的事、
前綴要整段對上而不是 `includes`）。它壞掉的方式是「安靜地少撈到東西」，不會自己吵，
所以上面那些斷言全都建立在它身上這件事需要一條獨立的迴歸網。
