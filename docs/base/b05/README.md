...
行為完全不變。這個界限只約束冷啟動與宣告變更；已在跑的排程遇到停機走的是 watermark 與 `catchUp`。
**宣告變更也算冷啟動**，所以改一個月排程的 cron 運算式，等於「從下一次開始」——被重設的
「當下這一次」若已陳舊超過七天就不補，最久要等一個月才第一次跑。這與「新的時間表不該重放舊的」
一致，但改宣告的人需要知道。冷啟動跳過的次數目前併入 `skipped_catchup`，維運看到它跳動時
要一併看 log 才分得出是停機太久還是這個政策。

停機後 `(watermark, now]` 之間的 occurrence 全部算出來，
...
排程沒有人會發現。（`start()` 的迴圈丟棄 tick 的回傳值，所以在 production 這個計數只有
`drain()` 與測試讀得到，實際的告警訊號是那個 warn／error 升級。）

**總預算的起點會輪替。** 預算是整個階段的總額，第一個排程在列鎖久等時就能自己吃滿；
起點若固定，順序靠後的排程會無限期排不到，而外面只看到「一個排程偶爾抖動」。所以這一輪
停在誰身上，下一輪就從誰開始。被延後的排程一次全部列進同一行 warn，連續延後同樣從 warn
升級成 error（連續三輪；「這一輪剛好撞上鎖等待」與「預算長期不夠」是兩種訊號），並計入
`WorkerTickResult.recurringDeferred`——延後是資源問題，與 `recurringFailed` 的「算不出結果」
分開計，混在一起會讓真的壞掉的訊號被慢排程蓋掉。這個計數器和 `recurringFailed` 一樣，
在 production 只有 log 讀得到。預算剩不到 250 毫秒就不再開新交易：`boundedTransaction`
逾時會把連線移出連線池，拿剩下的幾毫秒去開一個必定逾時的交易，代價是每輪毀一條連線。

**但每一輪至少排一個。** 預算本身小於一個切片時（只有繞過 zod 的建構子選項做得到），
若照門檻辦事就會每一輪都停在同一個位置、一個排程都不排，而且 `failed` 恆為 0——
那不是降級而是靜默停擺。所以順序上的第一個一律排，剩下的照門檻決定。
