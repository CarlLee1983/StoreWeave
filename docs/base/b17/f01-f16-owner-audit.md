...
| --- | --- | --- | --- |
| F01 runtime／DI／config／module graph | B01–B02；`kernel/runtime.ts`、`bundle/bootstrap.ts`、`config/schema.ts` | module-graph、release、runtime-lifecycle；B01/B02 evidence | local-reviewed；checksum-bound clean CI 已於 [`685b732` run](https://github.com/CarlLee1983/StoreWeave/actions/runs/34685750936) 達成並上傳四份 artifact，published artifact 仍 pending |
...
| F11 Cache／mutex | B11；`platform/cache`、Command bus 上的 `platform.cache.clearOpsCache`（`packages/platform/kernel/src/ops-module.ts:190`；`tools/cli` 沒有 cache 指令） | TTL/outage/cross-process/mutex termination；B11 acceptance | local-reviewed；權威資料不依賴 cache，production DB rollout 隨 release gate |
...
| F13 site shell／Theme／Admin mount | B13–B14；theme/page registry、site、storefront/Admin | missing renderer、permission-filtered navigation、Base storefront；B13 docs | local-reviewed；clean final matrix 已於 [`685b732` run](https://github.com/CarlLee1983/StoreWeave/actions/runs/34685750936) 在遠端跑過（10 job 全綠） |
