...
| secret redaction | 錯誤訊息只含 origin（path 會帶 webhook token）；request header 及不受信任的 fetch／stream error/cause 不進訊息；回應不交還 `set-cookie`；`hardening.test.ts` 與 `destinations.test.ts` 各有斷言 | implemented |
...
| crypto 的 sign／verify／encrypt／rotation 使用方式 | `signValue`／`verifySignedValue`／`encryptString`／`decryptString`；到期值以「加一把、所有 writer 改 active、等舊值到期、再移除」輪替；永久 MFA 密文以 `mfa:rewrap-secrets` 在新舊 key 共存時原子重封，重跑為 0 才移除舊 key | implemented |
...
| 目前整合 checkpoint | Sol/high reviewer 檢查 B12 完整差異後指出 NAT64 local-use／link-local 範圍、公開錯誤洩漏、malformed redirect、body／backoff 取消、預設 timeout timer、key id 驗證順序／clock，以及永久 MFA 密文輪替的原子性與操作流程。全部修正並補回歸與 `mfa:rewrap-secrets` CLI／runbook；同一 reviewer 複審 delta 與 affected callers/tests，結論無剩餘 actionable finding |

## 目前 checkpoint gates

目前未提交工作樹的 final-source 驗證：

| gate | 結果 |
| --- | --- |
| `make verify` typecheck | PASS |
| `make verify` Admin typecheck | PASS |
| `make verify` unit | 102 files／1,215 PASS |
| `make verify` Admin | 32 files／350 PASS |
| `make verify` integration | 106 files／898 PASS |
| `make verify` build／Admin build | PASS |
...
## B12 以外的 release gate

- B12 本機範圍已驗收，B09 已使用其簽章與時間契約。
- 真實 provider endpoint、merchant capability 與 UAT 不屬於本機 fixture 可證明的範圍，維持在 B17 acceptance matrix。
