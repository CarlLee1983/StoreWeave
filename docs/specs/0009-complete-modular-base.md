...
- 狀態：Base 工程交付完成（2026-09-19，B00–B17）；release readiness 與產品驗證分開追蹤，未宣稱 production-ready。
...
2026-09-10 確認跨應用方向：[通用應用基底與產品分工](../reusable-application-base.md)。
可復用範圍涵蓋後端、前端、CLI 與建置部署工具，後續產品可包含 ERP 等應用；
本規格既有能力矩陣與網站驗收仍保留，本文不新增完整 ERP 的交付要求。

StoreWeave 是供團隊交付與維護客製網站的模組化基底，先服務台灣單一商家的品牌購物站，
再以同版本的形象站與 Blog 驗證通用性。商品化先從建站與維護服務開始；價值是重用已驗證功能、
保留品牌與流程客製空間，並讓各站持續升級共用基底。

2026-09-19 確認本輪交付邊界：Base 工程完成表示基礎 implementation、呼叫端、文件與 repository
gates 已落地；本機 SMTP 設定也已完成實測。release readiness 指特定環境、供應商與正式設定的驗證；
產品驗證指真實案件的重用成本與商業結果。後兩者不阻擋 Base 工程完成，也不能由此推論系統已可正式上線。

Laravel／AdonisJS 是基礎服務完整度的演進參考。本輪以可運作的基礎工程與下列能力矩陣的已交付
implementation 為完成條件；可整合成熟套件，但套件本身能做，不等於 StoreWeave 已提供。
...
「完整」以本矩陣的 Base 工程交付為邊界，不等於逐一複製參考框架的所有生態產品。
...
下表的「現況」是 2026-09-06 的盤點基準；B00–B17 的交付結果與現行證據見
[B17 驗收矩陣](../base/b17/acceptance.md)。工作包編號見執行計畫。
...
- [x] F01–F16 每項都有正式 implementation、文件、故障測試與使用範例；[B17 owner audit](../base/b17/f01-f16-owner-audit.md) 與 `make verify` 記錄證據。
- [x] 同一 base 版本組出購物站、形象站、Blog。無電商組裝在乾淨 DB 無商務表、API、選單及必要設定，打包後也不載入 commerce runtime。
- [x] 更換兩個主題後，內容、媒體、URL、導覽及功能設定保留；主題專屬視覺可重設，購物站必需的頁面能力缺失在建置／啟動時被拒絕。
- [x] 一個有自己資料及前後台頁面的非電商模組，跑通「授權上傳 → 排 job → 背景處理 → 狀態查詢 → 寄信／站內通知」；worker 重啟與授權拒絕有驗證。
- [x] 新模組只修改模組及站點組裝宣告；模組契約測試驗證 missing dependency、版本不符、權限與命名衝突。
- [x] 既有購物流程、資料、事件、外部整合及 Admin 行為通過回歸；商家 UAT／正式服務設定與程式測試分開記錄，不能以 mock 代填。
- [x] 前一版資料＋排隊工作＋媒體能升級到新版，回復演練可重現；API／Worker 版本不一致可檢出。

### Release readiness（不阻擋 Base 工程）

- [ ] private media 與真實 transport 的 staging 驗證；目前 CI、Docker／native release smoke 與本機 SMTP 實測均已有證據。尚缺外部環境時，僅列為相應 release gate 未完成。
- [ ] 商家 UAT、正式服務設定與已發布 binary 的實際升級證據；它們限制相應的 release／production 主張，不回溯否定 Base 工程驗收。

### Quality／coverage（不阻擋 Base 工程）

- [ ] 公開契約 semantic ledger 的完整覆蓋；目前 560／703 個 facet 仍在 `remaining`，作為明確的品質工作清單，不宣稱 exhaustive coverage。
